...
public class MTEHatchMEDynamo extends MTEHatchDynamo implements IGridProxyable, IMEConnectable, IPowerChannelState {
    ...
    public long maxEUStore() {
        if (fixedAmperage == 2) {
            return Math.addExact(512L, Math.multiplyExact(V[mTier + 1], 2L));
        }
        return Math.addExact(512L, Math.multiplyExact(V[mTier], 16L));
    }
